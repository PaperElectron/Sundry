...
function readyStart() {
  if(--allReady === 0) {
    ...
    server.http.listen(serverOpts.port, serverOpts.bindAddress, function() {
      console.log('http proxy server started on port %s', serverOpts.port);
    });
    server.https.listen(serverOpts.sslPort, serverOpts.bindAddress, function() {
      console.log('https proxy server started on port %s.', serverOpts.sslPort);
    });

    if(server.test) {
      server.test.listen(10000, function() {
        console.log('Test http server started on port 10000.');
      });
    }
